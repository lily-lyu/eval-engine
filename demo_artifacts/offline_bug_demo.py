...
artifacts_dir = Path("demo_artifacts/offline_bug_arg_bad")
...
for line in src.read_text(encoding="utf-8").splitlines():
    if not line.strip():
        continue
    row = json.loads(line)
    if row.get("scenario_id") == "trajectory_wrong_tool_args":
        picked = row
        break

if not picked:
    raise SystemExit("Could not find trajectory_wrong_tool_args in break_suite.jsonl")

item = picked["item"]
oracle = picked["oracle"]
raw_output = picked["raw_output"]
tool_trace = picked["tool_trace"]
...
md = []
md.append("# Demo Report — Injected Wrong Tool Args")
md.append("")
