...
- **checker_name**: string or null (required for programmatic_check when the family has a checker).
...
- **evidence_requirements**: object or null; **required** when eval_method is `rubric_judge` (e.g. `{"required_evidence": ["reasoning", "citation"], "min_length": 1}`).
...
2. For **rubric_judge**, **evidence_requirements** must be present; do not omit or leave null. The engine does not allow rubric_judge without evidence.
3. **checker_name** must be a supported checker implementation for the family (from the catalog). Do not invent checker names that the engine does not implement.
4. **pass_fail_observables** must match the family’s **observable_targets**.
5. Do not select an eval_method that is not in the family’s allowed_eval_methods.
